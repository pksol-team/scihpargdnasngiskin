	...
	var lumise = {
			...
			lumise : {
				...
				'image' : function(ops, callback) {
					...
					img.onload = function() {
					   	...
					   	var src_data = lumise.fn.check_upload_dimensions(this);
					...
					general_status: function(e) {
						...
						switch (func) {
							...
							case 'cancel-design': 
								...
								lumise.fn.set_url('my_design', null);
						...
						switch (act) {
							...
							case 'edit' :
								...
								lumise.indexed.get(id, 'dumb', function(res){
									if (res !== null) {
										lumise.fn.set_url('cart', null);
										lumise.fn.set_url('my_design', id);
							...
							wrp.find('>span').on('click', function(){
								...
								var next_step = function() {
									...
									var psize = lumise.get.size(),
										...
										dataURL: new_group.toDataURL({multiplier: psize.h/lumise.stage().limit_zone.height})
									};
		...
		templates : {
			...
			load : function(tmp, callback) {
				...
				if (tmp.upload !== undefined && tmp.upload.indexOf('.lumi') > -1) {
				...
				} else if (tmp.upload !== undefined && tmp.upload.indexOf('.lumi') === -1) {
					...
					templ.src = tmp.upload.startsWith('https://') ? tmp.upload :   lumise.data.upload_url+tmp.upload;
		...
		fn : {
				...
				lumise.ops.update_image_fx_timer = setTimeout(function(){
					...
					if (!s.active.fxOrigin || !s.active.fxOrigin.tagName) {
						if (s.active.full_src) {
							...
							s.active.fxOrigin.src = s.active.full_src;
			...
			download_design : function (ops) {
				...
				switch (type) {
					...
					case 'json':
						...
						lumise.fn.download(
							'data:application/octet-stream;charset=utf-16le;base64,'+lumise.fn.enjson(data),
							name+'.lumi'
						);
			...
			edit_design : function(ops) {
				...
				lumise.tools.save();
				lumise.fn.set_url('my_design', ops.id);
			...
			export : function(save, id, created) {
				...
				if (
		    		...
		    		if (
		    			save == 'designs' || (
		    				lumise.ops.importing !== true && 
		    				lumise.fn.url_var('my_design', '') !== ''
		    			)
		    		) {
						
						var design_id = lumise.fn.url_var('my_design', ''),
		...
		render : {
			...
			refresh_my_designs : function(){
				...
				lumise.get.el('saved-designs').find('>li:not([data-id="new"])').remove();
			...
			my_designs : function(design){
				...
				var el = lumise.get.el('saved-designs'),
					cur = lumise.fn.url_var('my_design'),
					lis = '';
			...
			save_design : function(res) {
				...
				if (res.success) {
					...
					if ( window.location.href.indexOf('design='+res.id+'&') === -1)
						window.history.pushState({},"", lumise.data.url+'?design='+res.id+'&product_base='+res.pid);
					*/
				}else if(res.error) {
				...
				render : function(data) {
					...
					if (typeof data.values != 'object') {
						...
						if (typeof data.values == 'string') {
							...
							if (typeof values == 'object' && typeof values.options == 'object' && values.options.length > 0) {
		...
		load : function() {
			...
			*/
			...
			this.actions.add('ctrl-s', function(e) {
				...
				else return lumise.fn.export('designs');
			...
			this.actions.add('db-ready', function(){
				...
				} else if (lumise.fn.url_var('my_design', '') !== '') {
					var design_id = lumise.fn.url_var('my_design');
			...
			this.actions.add('first-completed', function(){
				...
				if (lumise.fn.url_var('cart', '') != '') {
					...
					if (lumise.fn.url_var('design_print', '') !== '') {
						...
						lumise.f('Loading..');
						var design_path =   lumise.apply_filters('print-design-url',lumise.data.upload_url );
						var url = (typeof design_path == lumise.data.upload_url) ? design_path : lumise.data.upload_url  +'designs/';
						$.ajax({
			...
			this.actions.add('save-design', function(id) {
				...
				lumise.get.el('general-status').html(
					'<span>\
						<text>\
							<i class="lumisex-android-alert"></i> '+
							lumise.i(188)+' '+lumise.fn.date('h:m - d M, Y', new Date().getTime())+'\
							<strong>#'+id+'</strong>\
						</text>\
						<a href="#cancel-design" data-btn="cancel" data-func="cancel-design">\
							'+lumise.i(187)+'\
				...
				if (lumise.fn.url_var('cart', '') === '')
					lumise.fn.set_url('my_design', id);
				else lumise.fn.set_url('my_design', null);
			...
			window.addEventListener('message', function(e) {
				...
				if (e.data && e.data.action) {
					switch (e.data.action) {
						...
						break;
						case 'add_image' : 
							lumise.fn.preset_import([{type: 'image', url: e.data.url}]);
						break;
						case 'preview_image': 
