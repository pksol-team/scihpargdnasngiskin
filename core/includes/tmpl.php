		...
		return <<<EOF
			...
			var content = '<div class="lumise-field-options-wrp rbd">\
					<p class="quantity-layout">\
						...
						<input type="text" name="min-qty" value="'+lumise.esc(values.min_qty)+'" />\
						<label>{$lumise->lang('Max Quantity')}: </label>\
						<input type="text" name="max-qty" value="'+lumise.esc(values.max_qty)+'" />\
					</p>';
				...
				if (typeof values == 'object' && values.multiple_options !== undefined && values.multiple_options.length > 0) {
					...
					values.multiple_options.map(function(item) {
						content += '<tr>\
								<td width="30%"><input type="text" name="title" value="'+lumise.esc(item.title)+'" /></td>\
								<td width="20%"><input type="text" name="price" value="'+lumise.esc(item.price)+'" /></td>\
								<td width="20%"><input type="text" name="min-qty" value="'+lumise.esc(item.min_qty)+'" /></td>\
								<td width="20%"><input type="text" name="max-qty" value="'+lumise.esc(item.max_qty)+'" /></td>\
				...
				if (typeof values == 'object' && values.package_options !== undefined && values.package_options.length > 0) {
					...
					values.package_options.map(function(item) {
						content += '<tr>\
								<td width="50%"><input type="text" name="title" value="'+lumise.esc(item.title)+'" /></td>\
								<td width="20%"><input type="text" name="value" value="'+lumise.esc(item.value)+'" /></td>\
								<td width="20%"><input type="text" name="price" value="'+lumise.esc(item.price)+'" /></td>\
								<td width="10%"><i class="fa fa-times" data-func="delete"></i></td>\
							</tr>';
					});
			...
			wrp.export_value = function() {
				var values = {
					type: wrp.find('select[name="layout"]').val(),
					min_qty: wrp.find('input[name="min-qty"]').val(),
					max_qty: wrp.find('input[name="max-qty"]').val(),
					multiple_options: [],
					package_options: []
				};
				wrp.find('table.lumise-field-options.multiple_options tbody tr').each(function() {
					if ($(this).find('input[name="title"]').val() !== '') {
						values.multiple_options.push({
							value: lumise.slugify($(this).find('input[name="title"]').val()),	
							title: $(this).find('input[name="title"]').val(),	
							price: $(this).find('input[name="price"]').val(),
							min_qty: $(this).find('input[name="min-qty"]').val(),
							max_qty: $(this).find('input[name="max-qty"]').val()
						});
					}
				});
			...
			trigger({
				...
				events: {
					'select[name="layout"]:change': 'layout',
					'input[name="min-qty"]:change,input[name="max-qty"]:change': 'qty',
				...
				add_multiple: function(e) {
					...
					var row = $('<tr>\
							<td width="30%"><input type="text" name="title" value="" /></td>\
							<td width="20%"><input type="text" name="price" value="" /></td>\
							<td width="20%"><input type="text" name="min-qty" value="" /></td>\
							<td width="20%"><input type="text" name="max-qty" value="" /></td>\
							<td width="10%"><i class="fa fa-times" data-func="delete"></i></td>\
						</tr>');
				...
				add_package: function(e) {
					...
					var row = $('<tr>\
							<td width="50%"><input type="text" name="title" value="" /></td>\
							<td width="20%"><input type="text" name="value" value="" /></td>\
							<td width="20%"><input type="text" name="price" value="" /></td>\
							<td width="10%"><i class="fa fa-times" data-func="delete"></i></td>\
						</tr>');
