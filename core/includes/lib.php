...
class lumise_lib{
	...
	*/
    ...
    public function pdf_download($id) {
		...
		if (is_file($this->main->cfg->upload_path. 'user_data'.DS.'pdfs'.DS.$id.'.pdf')) {
			header('location: '.$this->main->cfg->upload_url. 'user_data/pdfs/'.$id.'.pdf');
		}
		...
		if (count($files) > 0) {
			...
			if (!is_dir($this->main->cfg->upload_path. 'user_data'.DS.'pdfs')) {
				@mkdir($this->main->cfg->upload_path. 'user_data'.DS.'pdfs', 0755);	
			}
			...
			foreach ($files as $image) {
			    if ($combined == null) {
			        $combined = new Imagick($image);
					$combined->trimImage ( 20 ); 
			    }
			    else {
			        $page = new Imagick($image);
			        $combined->addImage($page);
			    }
			}
			
			$combined->setImageFormat("pdf");
			$combined->writeImages($this->main->cfg->upload_path. 'user_data'.DS.'pdfs'.DS.$id.'.pdf', true );
